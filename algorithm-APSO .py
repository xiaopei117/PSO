...
output_file = "APSO_algorithm.xlsx"
...
class PSO(object):
    ...
    def particle_init(self,e1,e2):
        ...
        # (5)Z_lc,Z_cb
        a5 = np.zeros((self.dim1, self.dim2))      # a5对应Z_lc——252*70
        for a, adx1 in enumerate(a1):              # 把a3矩阵中，位置为i、值为idx的元素
            a5[a][adx1 - 1] = 1                    # a5:0/1 (252,70)
        z_lc = a5
    ...
    def pso_third(self, N_, g_best_index, t, f_state):
        ...
        w = self.w_max - (t / self.T) * (self.w_max - self.w_min) * (np.e ** (-abs(f_))) * f_
    ...
    def pso_main(self):
        ...
        for o in range(self.S):
            ...
            N.append(x)
            x_1 = np.concatenate((b1, b2, a1, a3), axis=0)
            N_1.append(x_1)
            # (3 求初代的适应度
            fitness1 = self.fun1(z_cb, z_lc, u_bs, u_cs, a24, a27, a1, a21, a41, z_cb_, u_bs_, a27_)
        ...
        for tt in tqdm(range(0,self.T)):                            # 每一次迭代都有进度条显示
            ...
            # 2）得迭代后的关键参数
                ...
                b1, b2, choice1, choice2, z_lc, z_cb, u_bs, u_cs, a24, a27, a1, a3, a21, a41, z_cb_, u_bs_, a27_ = self.particle_init(e1,e2)
                x_2 = np.concatenate((b1, b2, a1, a3), axis=0)
                N_2[jj] = x_2
            # 3）得迭代后的适应度
            ...
            for ii in range(self.S):
                if np.random.rand() <= 0.9:
                    ...
                    # 2) 交叉——将[parent1奇数位]+[=parent2偶数位]进行交叉
            ...
            for nn in range(self.S):
                # 1.得迭代后的关键参数
                e2 = N3[nn][:self.dim3]
                e1 = N3[nn][self.dim3: self.dim3 + self.dim2]
                e1 = e1.astype(np.int16)
                b1, b2, choice1, choice2, z_lc, z_cb, u_bs, u_cs, a24, a27, a1, a3, a21, a41, z_cb_, u_bs_, a27_  = self.particle_init(e1,e2)
                x_3 = np.concatenate((b1,b2,a1,a3), axis=0)
                N_3[nn] = x_3
                # 3）得迭代后的适应度
                fitness3 = self.fun1(z_cb, z_lc, u_bs, u_cs, a24, a27, a3, a21, a41, z_cb_, u_bs_, a27_ )
            ...
            # (7) 个体最优值列表替换——将个体最优值存在self.num_N里
            ...
            # (9) 求全局最优值——拿这一代的最优值跟全局最优做比较,若这一代最优值更优，则成为全局最优值
            ...
            pg_best_state.append(pg.copy())                          # 全局最优粒子位置列表
            ...
            f_vary = self.P_min + (self.P_max - self.P_min) * (1 - (f_vary / self.S))
            print("f_vary", f_vary)
            pg_ = pg
            if np.random.rand() <= f_vary:
                d1 = random.sample(range(0, self.dim3), 3)
                for x in d1:
                    pg_[x] = np.random.uniform(0, self.dim5, (1))
            # 1.得迭代后的关键参数
            e2 = pg_[:self.dim3]
            e1 = pg_[self.dim3: self.dim3 + self.dim2]
            e1 = e1.astype(np.int16)
            b1, b2, choice1, choice2, z_lc, z_cb, u_bs, u_cs, a24, a27, a1, a3, a21, a41, z_cb_, u_bs_, a27_  = self.particle_init(e1, e2)
            # 2）得迭代后的适应度
            fitness4 = (self.fun1(z_cb, z_lc, u_bs, u_cs, a24, a27, a3, a21, a41, z_cb_, u_bs_, a27_ )).squeeze().item()
